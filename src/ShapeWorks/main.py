...
def preprocessing(args):
    ...
    for segmentation in tqdm(glob(f"{data_dir}*.nii")):
        filename = segmentation.split(".")[0]
        ...
        mesh_left.write(f"{filename}_left.vtk")
        mesh_right.write(f"{filename}_right.vtk")
...
def grooming(args):
    ...
    mesh_files = glob(f"{args.data_dir}*.vtk")
